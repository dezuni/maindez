...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تابع برای غیرفعال کردن کارت وقتی زمان به پایان می‌رسد
    function disableCard(cardId) {
        const voucherCard = document.querySelector(`.voucher-card:has(#timer-container-${cardId})`);
        if (voucherCard) {
            // اضافه کردن کلاس dimmed برای محو کردن کارت
            voucherCard.classList.add('dimmed');
            
            // پیدا کردن دکمه رزرو و غیرفعال کردن آن
            const reserveBtn = voucherCard.querySelector('.deposit-link');
            if (reserveBtn && !reserveBtn.classList.contains('disabled')) {
                // جایگزینی لینک با دکمه غیرفعال
                reserveBtn.outerHTML = '<div class="deposit-link disabled">غیرفعال</div>';
            }
            
            console.log(`کارت ${cardId} غیرفعال شد`);
        }
    }
